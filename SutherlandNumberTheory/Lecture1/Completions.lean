...
/-- A local instance of `IsPrime` for `(p : ℤ)` that
`padicValuationRing` below can use to synthesize
`Localization.AtPrime`. -/
instance spanSingletonIntPrime_isPrime (p : ℕ)
    [Fact (Nat.Prime p)] :
    (Ideal.span ({(p : ℤ)} : Set ℤ)).IsPrime :=
  primeIdealZ_isPrime p

/-- The valuation ring `ℤ_(p)` of the `p`-adic
valuation on `ℚ`, modeled as the localization of `ℤ`
at the prime ideal `(p)`. -/
abbrev padicValuationRing (p : ℕ)
    [Fact (Nat.Prime p)] : Type :=
  Localization.AtPrime
    (Ideal.span ({(p : ℤ)} : Set ℤ))

/-- The maximal ideal of `ℤ_(p)` is generated by the
image of `p`. -/
theorem padicValuationRing_maximalIdeal_eq_span_p
    (p : ℕ) [Fact (Nat.Prime p)] :
    IsLocalRing.maximalIdeal (padicValuationRing p) =
      Ideal.span
        ({algebraMap ℤ (padicValuationRing p) p}
          : Set (padicValuationRing p)) := by
  have h :
      IsLocalRing.maximalIdeal (padicValuationRing p) =
        (Ideal.span ({(p : ℤ)} : Set ℤ)).map
          (algebraMap ℤ (padicValuationRing p)) :=
