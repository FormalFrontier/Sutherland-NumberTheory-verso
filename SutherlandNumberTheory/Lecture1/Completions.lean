...
A _valuation_ on a field $k$ is a group homomorphism $k^{\times} \to \mathbb{R}$ such that for all $x, y \in k$ we have

$$v(x + y) \geq \min\bigl(v(x), v(y)\bigr).$$

We may extend $v$ to a map $k \to \mathbb{R} \cup \{\infty\}$ by defining $v(0) \coloneqq \infty$. For any $0 < c < 1$, defining $\lvert x \rvert_v \coloneqq c^{v(x)}$ yields a nonarchimedean absolute value. The image of $v$ in $\mathbb{R}$ is the _value group_ of $v$. We say that $v$ is a _discrete valuation_ if its value group is equal to $\mathbb{Z}$ (every discrete subgroup of $\mathbb{R}$ is isomorphic to $\mathbb{Z}$, so we can always rescale a valuation with a discrete value group so that this holds). Given a field $k$ with valuation $v$, the set
...
A _valuation ring_ is an integral domain $A$ with fraction field $k$ with the property that for every $x \in k$, either $x \in A$ or $x^{-1} \in A$.
...
A _local ring_ is a commutative ring with a unique maximal ideal.
...
The _residue field_ of a local ring $A$ with maximal ideal $\mathfrak{m}$ is the field $A/\mathfrak{m}$.
...
For the $p$-adic valuation $v_p \colon \mathbb{Q} \to \mathbb{Z} \cup \{\infty\}$ we have the valuation ring
...
For any field $k$, the valuation $v \colon k((t)) \to \mathbb{Z} \cup \{\infty\}$ on the field of Laurent series over $k$ defined by
...
_For an integral domain $A$, the following are equivalent:_
