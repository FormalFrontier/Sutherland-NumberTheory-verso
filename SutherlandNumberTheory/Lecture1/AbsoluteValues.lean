...
**Definition 1.2.** An _absolute value_ on a field $k$ is a map $\lvert \cdot \rvert \colon k \to \mathbb{R}_{\geq 0}$ such that for all $x, y \in k$ the following hold:
...
**Definition 1.6.** Two absolute values $\lvert \cdot \rvert$ and $\lvert \cdot \rvert'$ on the same field $k$ are _equivalent_ if there exists an $\alpha \in \mathbb{R}_{>0}$ for which $\lvert x \rvert' = \lvert x \rvert^{\alpha}$ for all $x \in k$.
